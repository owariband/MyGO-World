...
LangChain Core（内部依赖）
  └── Runnable、调用配置以及未来 ChatModel/Prompt/Tool 接入

World Runtime（ADK 外部权威，待继续实现）
  └── Scheduler、Director、Validator、Committer、Ledger
```
...
Creator agents.json（不受信）
  ...
  -> Director -> Binder -> Validator -> Committer（待实现）
  -> committed WorldSegment / WorldEvent（待实现）
```

必须区分当前事实：

- **已实现的基础：**Manifest 严格读取、受信编译、Catalog 权限收敛、稳定 digest、Persona 私有 Memory/State 的部分基础能力，以及固定 `ActionProposal` envelope + discriminated action union 的 World-owned contract；
...
- **尚未实现：**长期 Memory Store、Director/Binder/Committer、完整咖啡 Golden Trace、真实 ChatModel、Dynamic Render ingress、热更新和 UI；
- **不变的权威边界：**外部 Event Scheduler 拥有循环；PersonAct 只能产生 `ActionProposal`，不能修改 World、Ledger、其他角色 Memory 或 Render 状态。
...
| 策略替换点 | 项目定义的 `CognitionStrategy` Protocol | Fixture 与未来 ChatModel 使用相同 typed seam |
| 模型/Prompt/Tool | LangChain Core 接口，尚未接入生产模型 | 后续只替换受信策略，不扩张 Manifest 权限 |
...
- World Commit、Ledger 或 Render 副作用；
...
4. 模型/Tool 输出未来也必须先显式 parse 成 strict Pydantic Model；
...
`decide` 是一次短、有界的决策调用，没有必要为它引入图状态、checkpoint 与迁移成本。当前内部使用 typed Runnable 组织阶段，但它不拥有外部循环。若未来出现经过验证的复杂分支、暂停恢复或长生命周期执行需求，再单独评估 LangGraph。即使引入，图状态也不能替代 Event Scheduler 循环、Persona 长期 Memory、World Snapshot、Ledger 或 commit protocol。

## 创作者配置

MVP 在项目内约定 `projects/<project-id>/agents.json`。创作者可以配置 Persona、初始私有信念、受限 Proposal 类型、受信 Tool 引用、有限行为参数和 Prompt Profile：
...
- WorldSnapshot、Ledger 或 world-version 写权限；
- model/provider/API key；
- URL、请求头、脚本、任意函数或任意 MCP server；
- Committer、Render Gateway 或 WebGAL 命令；
- Fixture/生产模式选择；
- Runnable/Graph 拓扑。

Fixture 与真实模型的绑定属于运行环境，不能由创作者越权选择。
...
4. Prompt 只能引用精确受信版本，不能内联 system prompt；
5. 超预算报错，不静默截断；
6. canonical digest 写入 Trace，一个 Runtime Session 固定一个 digest；
7. MVP 不写第二份 compiled 文件，避免源配置与生成物漂移。
...
Event Scheduler 选择 agent + committed frame
  ...
  -> 返回 Scheduler / Director / Validator / Committer 链
```
...
这些检查仍只是 Agent 边界。Proposal 通过后还要经过 Director、Temporal Binder、World Validator 和 Committer；这些完整链路当前尚未实现。
...
- Director、Binder、Validator、Committer；
- 真实模型输出质量、重试与 repair；
- 多 Event 调度和跨实体隔离；
...
3. 完成咖啡场景的 Director Fixture、Binder、Validator、Committer 和 Golden Trace；
4. 以相同契约只把 `decide` 内受信 strategy 替换为真实模型 shadow run；
5. shadow 稳定后再允许 gated commit，语义 repair 最多一次并留 Trace；
6. 再分别实现 Director 与 Broadcast，不复制 PersonAct 内部拓扑；
...
代码落地后按 `AGENT.md` 运行格式、lint、pyright strict 与 pytest；本次只改文档，不宣称这些检查已通过。
