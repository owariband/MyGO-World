"""Stateful PersonAct cognition ending at one validated action proposal."""
...
class _DecisionResult(StrictModel):
...
class PersonActAgent:
    """Run Persona cognition and return only the Persona's typed action.

    Calls for one Persona are serialized. Its private snapshot is replaced
    ...
    def __init__(
        ...
        prepare_node: Runnable[DecisionRequest, _PreparedDecision] = RunnableLambda(
            self._prepare,
            name="prepare",
        )
        ...
        propose_node: Runnable[_PlannedDecision, _DecisionResult] = RunnableLambda(
            self._propose,
            name="propose",
        )
        self._runnable: Runnable[DecisionRequest, _DecisionResult] = RunnableSequence[
            DecisionRequest,
            _DecisionResult,
        ](
            first=prepare_node,
    ...
    def decide(
        ...
        with self._lock:
            ...
            result = self._runnable.invoke(validated_request, config=config)
            try:
                validated_result = _DecisionResult.model_validate(result, strict=True)
            except ValidationError as error:
                raise PlannerOutputError("decision returned an invalid private result") from error

            self._snapshot = _PrivateSnapshot(
                state=validated_result.state,
                memory=validated_result.memory,
                trace=validated_result.trace,
                # Only the immediately preceding decision is cached. Older
                # proposal-id replay belongs to the external decision ledger.
                replay=_ReplayRecord(
                    proposal_id=validated_request.proposal_id,
                    request_fingerprint=request_fingerprint,
                    proposal=validated_result.proposal,
                    trace=validated_result.trace,
                ),
                # The compact set prevents an older id from being executed
                # again. The external decision ledger will own durable replay.
                used_proposal_ids=(
                    self._snapshot.used_proposal_ids | {validated_request.proposal_id}
                ),
            )
            return validated_result.proposal
    ...
    def _prepare(self, request: DecisionRequest) -> _PreparedDecision:
        frame = request.frame
        ...
        if (
            self._snapshot.state.last_world_time is not None
            and frame.world_time < self._snapshot.state.last_world_time
        ):
            raise DecisionInputError("frame.worldTime cannot move Persona state backwards")
        if (
            self._snapshot.state.last_world_version is not None
            and frame.based_on_world_version < self._snapshot.state.last_world_version
        ):
            raise DecisionInputError("frame world version cannot move Persona state backwards")
        if any(
            record.last_accessed_at > frame.world_time for record in self._snapshot.memory.records
        ):
        ...
        new_day = calculate_new_day(self._snapshot.state.last_world_time, frame.world_time)
    ...
    def _propose(self, decision: _PlannedDecision) -> _DecisionResult:
