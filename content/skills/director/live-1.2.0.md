...
对于 `utterance`，原样保留 `text`、`addressee_ids`、`expects_response` 和
`response_to_event_id`。从输入的 `world_time_ms` 开始安排时间，事件之间不得重叠，
总时长不超过五分钟。只有确有因果关系时，才在 `cause_event_keys` 中引用更早的事件。
